...
BATCH, D, BLOCK = U.BATCH, U.EMB_DIM, 256
HF = float(os.environ.get('HF', '0.01'))
...
sd = torch.load(os.environ.get('ORD_MODEL','models/gpu24/dlrm_terabyte_24day.pt'), map_location='cpu', weights_only=False)['state_dict']
...
def pcs(x, k=2):
    ...
    g = torch.Generator().manual_seed(0); vs=[]
    for i in range(k):
...
def make_order(c, how):
    if how == 'none':  return torch.arange(c.shape[0])
    if how == 'value': return torch.argsort(c.mean(dim=1))
    p1, p2 = pcs(c)
    q = lambda x: ((x - x.min())/(x.max()-x.min()+1e-12)*65535).long()
    if how == 'pc1':    return torch.argsort(p1)
    if how == 'zorder': return torch.argsort(morton(q(p1), q(p2)))
    if how == 'pc1pc2': return torch.argsort(q(p1)//4096 * 100_000_000 + q(p2))
...
results={}
import os as _os
ORDERINGS = _os.environ.get('ORDERINGS','value,none').split(',')
for sort in ORDERINGS:
    ...
    # timing: 30 warmup + 300 timed batches, full forward
    import time as _t
    it = iter(test_ld); times=[]
    for _ in range(30):
        X,o,i,T = next(it)
        if X.shape[0]==BATCH: f(X,o,i)
    while len(times) < 300:
        X,o,i,T = next(it)
        if X.shape[0]!=BATCH: continue
        _s=_t.perf_counter(); f(X,o,i); times.append((_t.perf_counter()-_s)*1000)
    times=np.array(times)
    a=auc_of(f)
    results[sort]=dict(auc=a, mean_ms=float(times.mean()), p50_ms=float(np.percentile(times,50)),
                       p99_ms=float(np.percentile(times,99)), std_ms=float(times.std()))
    log(f"perdim bs256 @{HF:.3f} order={sort:<8} AUC {a:.6f}  "
        f"fwd mean {times.mean():6.3f} p50 {np.percentile(times,50):6.3f} "
        f"p99 {np.percentile(times,99):6.3f} std {times.std():5.3f} ms")
json.dump(results, open(os.environ.get('ORD_OUT','results/perdim_order_ablation.json'),'w'), indent=1)
log("done")
