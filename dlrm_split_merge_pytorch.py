...
class DLRM_Net(nn.Module):
    ...
    def _group_tables_by_size(self, ln, merge_threshold, split_threshold, min_table_size):
        """
        Categorize tables into groups based on size thresholds.
        """
        to_merge = []
        to_split = []
        standalone = []
        ...
        for i in range(0, ln.size):
            ...
            # Categorize by size
            if split_threshold > 0 and n >= split_threshold and not needs_special_treatment:
                large_count += 1
                to_split.append((i, n))
                print(f"[GROUP] Feature C{i+1} (size={n}): marked for SPLIT")
            elif merge_threshold > 0 and n >= min_table_size and n < merge_threshold and not needs_special_treatment:
                small_count += 1
                to_merge.append((i, n))
                print(f"[GROUP] Feature C{i+1} (size={n}): marked for MERGE")
            else:
                if n < min_table_size:
                    tiny_count += 1
                    reason = "too small"
                elif needs_special_treatment:
                    reason = "needs QR/MD"
                else:
                    reason = "medium size"
                standalone.append((i, n))
                print(f"[GROUP] Feature C{i+1} (size={n}): STANDALONE ({reason})")
    ...
    def _create_split_tables(self, m, ln, weighted_pooling, split_threshold, tables_to_split, num_splits=4, start_idx=0):
        """
        Split large tables into smaller sub-tables to reduce memory pressure per table.
        ...
        if not tables_to_split:
            return emb_list, v_W_list, index_map
        
        print(f"\n[SPLIT] Splitting {len(tables_to_split)} large tables into {num_splits} sub-tables each")
        
        for orig_idx, total_size in tables_to_split:
            # Calculate split sizes (roughly equal)
            ...
            print(f"[SPLIT] Table C{orig_idx+1} (size={total_size}): splitting into {num_splits} tables, sizes={split_sizes}")
            
            # Track all sub-tables for this feature
            sub_table_indices = []
            offset = 0
            
            for split_num, split_size in enumerate(split_sizes):
                # Create sub-table
                if self.qr_flag and split_size > self.qr_threshold:
                    print(f"[SPLIT]   -> Sub-table {split_num}: Using QR embedding (size={split_size})")
                    EE = QREmbeddingBag(
                        split_size, m, self.qr_collisions,
                        operation=self.qr_operation,
                        mode="sum", sparse=True,
                    )
                elif self.md_flag and split_size > self.md_threshold:
                    print(f"[SPLIT]   -> Sub-table {split_num}: Using MD embedding (size={split_size})")
                    base = max(m) if isinstance(m, list) else m
                    _m = m[orig_idx] if isinstance(m, list) else base
                    EE = PrEmbeddingBag(split_size, _m, base)
                    W = np.random.uniform(
                        low=-np.sqrt(1 / split_size), 
                        high=np.sqrt(1 / split_size), 
                        size=(split_size, _m)
                    ).astype(np.float32)
                    EE.embs.weight.data = torch.tensor(W, requires_grad=True)
                else:
                    print(f"[SPLIT]   -> Sub-table {split_num}: Using standard EmbeddingBag (size={split_size})")
                    EE = nn.EmbeddingBag(split_size, m, mode="sum", sparse=True)
                    W = np.random.uniform(
                        low=-np.sqrt(1 / split_size), 
                        high=np.sqrt(1 / split_size), 
                        size=(split_size, m)
                ...
                sub_table_indices.append((current_table_idx, offset))
                offset += split_size
                
                emb_list.append(EE)
                v_W_list.append(None if weighted_pooling is None else torch.ones(split_size, dtype=torch.float32))
            
            # Store list of sub-tables for this feature
            index_map[orig_idx] = sub_table_indices
    ...
    def create_emb(self, m, ln, weighted_pooling=None, merge_threshold=0, split_threshold=0, 
                min_table_size=50, num_splits=4):
        """
        Create embedding tables with optional merging and splitting strategies.
        
        Args:
            m: embedding dimension
            ln: array of table sizes
            weighted_pooling: weighted pooling configuration
            merge_threshold: size threshold for merging (0 = no merging)
            split_threshold: size threshold for splitting (0 = no splitting)
            min_table_size: minimum size for tables to be considered for merging
            num_splits: number of sub-tables to split each large table into
        ...
        # If no merging or splitting, use original logic
        if merge_threshold == 0 and split_threshold == 0:
        ...
        groups = self._group_tables_by_size(ln, merge_threshold, split_threshold, min_table_size)
        ...
        # In create_emb function, replace the section where you create tables:

        # Create all tables
        emb_l = nn.ModuleList()
        v_W_l = []
        current_idx = 0  # Track current position in the global embedding list

        # 1. Create merged tables
        if merge_threshold > 0 and groups['to_merge']:
            ...
            current_idx += len(merged_emb)  # Update current index

        # 2. Create split tables
        if split_threshold > 0 and groups['to_split']:
            split_emb, split_v_W, split_map = self._create_split_tables(
                m, ln, weighted_pooling, split_threshold, groups['to_split'], num_splits, start_idx=current_idx
            )
            emb_l.extend(split_emb)
            v_W_l.extend(split_v_W)
            self.emb_index_map.update(split_map)
            current_idx += len(split_emb)  # Update current index

        # 3. Create standalone tables
    ...
    def __init__(
        ...
        if (
            ...
            # create operators
            if ndevices <= 1:
                self.emb_l, w_list = self.create_emb(m_spa, ln_emb, weighted_pooling,merge_threshold,split_threshold)
    ...
    def apply_emb(self, lS_o, lS_i, emb_l, v_W_l):
        ...
        for k, sparse_index_group_batch in enumerate(lS_i):
            ...
            if isinstance(emb_mapping, list):
                # === SPLIT TABLE (your code works fine) ===
                ...
                for sub_idx, (table_idx, base_offset) in enumerate(emb_mapping):
                    ...
                    if mask.any():
                        ...
                        V_part = torch.zeros(batch_size, E.embedding_dim, dtype=E.weight.dtype, device=E.weight.device)
                ...
                # === MERGED or STANDALONE TABLE ===
                # Simple approach: just do a lookup with adjusted indices
                # Even if multiple features share this table, each does its own lookup
                # The memory locality benefit comes from the table being merged in memory
                ...
                if self.quantize_emb:
                    s1 = self.emb_l_q[table_idx].element_size() * self.emb_l_q[table_idx].nelement()
                    s2 = self.emb_l_q[table_idx].element_size() * self.emb_l_q[table_idx].nelement()
