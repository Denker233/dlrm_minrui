#!/usr/bin/env python3
"""
Retrained Model: Hot/Cold Embedding Splitting with Differential Compression
Uses the fully-trained 1-epoch model for Phase 2B of the retrain task.
...
def update_status(phase_text):
    try:
        ...
        pass
    print(f"[STATUS] {phase_text[:80]}", flush=True)
...
def write_access_profile(profiles, total_emb_size, baseline_acc, baseline_auc,
    ...
    path = os.path.join(RESULTS_DIR, "retrained_hotcold_access_profile.md")
    large = [p for p in profiles if p['is_large']]
    small = [p for p in profiles if not p['is_large']]

    with open(path, 'w') as f:
        f.write("# Hot/Cold Access Pattern Profile (Retrained Model)\n\n")
        f.write("## Model Summary\n\n")
        f.write(f"- **Model:** `{MODEL_PATH}` (1 full epoch)\n")
...
def write_compression_results(results, baseline_acc, baseline_auc, total_emb_size):
    path = os.path.join(RESULTS_DIR, "retrained_hotcold_results.md")
    with open(path, 'w') as f:
        f.write("# Hot/Cold Differential Compression Results (Retrained Model)\n\n")
...
def write_vs_uniform(combo_results, uniform, baseline_acc, baseline_auc,
    ...
    path = os.path.join(RESULTS_DIR, "retrained_hotcold_vs_uniform.md")
    ...
    with open(path, 'w') as f:
        f.write("# Hot/Cold vs Uniform Compression Comparison (Retrained Model)\n\n")
...
def main():
    ...
    print("=" * 80)
    print("RETRAINED MODEL: HOT/COLD EMBEDDING SPLITTING EXPERIMENT")
    print("=" * 80)

    # Load model and data
    update_status("### Phase 2B: Loading retrained model and data...")
    ...
    print("=" * 80)
    update_status("### Phase 2B.1: Profiling access patterns — IN PROGRESS")
    ...
    update_status(f"### Phase 2B.1: Access profiling — DONE ({n_large} large, {num_tables - n_large} small)")
    ...
    print("=" * 80)
    update_status("### Phase 2B.2: Pre-compressing subtables — IN PROGRESS")
    ...
    update_status("### Phase 2B.2: Pre-compression — DONE")
    ...
    for hot_crf in HOT_CRFS:
        for cold_crf in COLD_CRFS:
            ...
            update_status(f"### Phase 2B.3: Combination {combo_num}/{total_combos} "
    ...
    update_status("### Phase 2B.3: All 25 combinations — DONE")
    ...
    print("=" * 80)
    update_status("### Phase 2B.4: Uniform CRF 23 comparison — IN PROGRESS")
    ...
    update_status("### Phase 2B.4: Uniform comparison — DONE")
    ...
    print("=" * 80)
    update_status("### Phase 2B.5: Writing reports — IN PROGRESS")
    ...
    for fname in ["retrained_hotcold_access_profile.md",
                  "retrained_hotcold_results.md",
                  "retrained_hotcold_vs_uniform.md"]:
    ...
    update_status(
        f"### Phase 2B: Hot/Cold experiment — DONE\n"
        f"- Best AUC: {best_auc['auc']:.6f} (hot={best_auc['hot_crf']}, cold={best_auc['cold_crf']}, {best_auc['compression_ratio']:.1f}x)\n"
        f"- Best ratio: {best_ratio['compression_ratio']:.1f}x (hot={best_ratio['hot_crf']}, cold={best_ratio['cold_crf']}, AUC={best_ratio['auc']:.6f})")

    send_notification(
        f"Phase 2B DONE (retrained)! "
        f"Best AUC: {best_auc['auc']:.6f} (H{best_auc['hot_crf']}/C{best_auc['cold_crf']}), "
        f"Best ratio: {best_ratio['compression_ratio']:.1f}x "
        f"(H{best_ratio['hot_crf']}/C{best_ratio['cold_crf']})")

    print("\n" + "=" * 80)
    print("HOT/COLD EXPERIMENT COMPLETE (RETRAINED MODEL)")
    print("=" * 80)
