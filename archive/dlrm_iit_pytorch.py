...
def extract_memory_usage(profile_data, operator_name):
    for line in profile_data.splitlines():
        if operator_name in line:
            match = re.search(r"[+-]\d+\.\d+\s\wB", line)
            if match:
                return match.group()
    return None


def convert_to_kb(memory_usage):
    match = re.match(r"([+-]?\d+\.\d+)\s(\wB)", memory_usage)
    if match:
...
def print_memory_usage(profile_data):
    # Extract memory usage for embedding-related operators
    embedding_bag_mem = extract_memory_usage(profile_data, "aten::embedding_bag")
    embedding_bag_backward_mem = extract_memory_usage(profile_data, "autograd::engine::evaluate_function: EmbeddingBagBac...")

    # Sum the embedding memory usage
    total_embedding_mem = 0
    if embedding_bag_mem:
        total_embedding_mem += convert_to_kb(embedding_bag_mem)
    if embedding_bag_backward_mem:
        total_embedding_mem += convert_to_kb(embedding_bag_backward_mem)

    # Extract memory usage for other operators
    dlrm_forward_mem = extract_memory_usage(profile_data, "DLRM forward")
    dlrm_backward_mem = extract_memory_usage(profile_data, "DLRM backward")

    addmm_mem = extract_memory_usage(profile_data, "aten::addmm")
    bmm_mem = extract_memory_usage(profile_data, "aten::bmm")
    relu_mem = extract_memory_usage(profile_data, "aten::relu")
    bmm_backward_mem = extract_memory_usage(profile_data, "autograd::engine::evaluate_function: BmmBackward0")
    addmm_backward_mem = extract_memory_usage(profile_data, "autograd::engine::evaluate_function: AddmmBackward0")
    relu_backward_mem = extract_memory_usage(profile_data, "autograd::engine::evaluate_function: ReluBackward0")

    # Print memory usage for all operators
    print(f"Memory usage for aten::embedding_bag: {embedding_bag_mem}")
    print(f"Memory usage for EmbeddingBagBackward: {embedding_bag_backward_mem}")
    print(f"Total embedding memory usage: {total_embedding_mem:.2f} KB")
    print(f"Memory usage for DLRM forward: {dlrm_forward_mem}")
    print(f"Memory usage for DLRM backward: {dlrm_backward_mem}")
    print(f"Memory usage for aten::addmm: {addmm_mem}")
    print(f"Memory usage for aten::bmm: {bmm_mem}")
    print(f"Memory usage for aten::relu: {relu_mem}")
    print(f"Memory usage for BmmBackward: {bmm_backward_mem}")
    print(f"Memory usage for AddmmBackward: {addmm_backward_mem}")
    print(f"Memory usage for ReluBackward: {relu_backward_mem}")
...
class DLRM_Net(nn.Module):
    ...
    def interact_features(self, x, ly):
        ...
        process = psutil.Process(os.getpid())
        before = process.memory_info().rss / 1e6  # Convert to MB
        tracemalloc.start()
        before, peak = tracemalloc.get_traced_memory()
        ...
        tracemalloc.stop()
        end_time = time.time() 
        after = process.memory_info().rss / 1e6  # Convert to MB
        # Calculate the elapsed time
        elapsed_time = end_time - start_time
        self.time_interact+=elapsed_time
        self.mem_interact+= peak - before
...
def run():
    ...
    global last_mem_look_up 
    global last_mem_mlp
    global last_mem_interact
    global dlrm
            ...
            while k < args.nepochs:
                ...
                for j, inputBatch in enumerate(train_ld):
                    ...
                    if should_print or should_test:
                        ...
                        mem_diff_look_up = (dlrm.mem_look_up - last_mem_look_up) / total_iter
                        mem_diff_mlp = (dlrm.mem_mlp - last_mem_mlp) / total_iter
                        mem_diff_interaction = (dlrm.mem_interact - last_mem_interact) / total_iter
                        print(
                        ...
                        print(f"Memory differences: lookup {mem_diff_look_up:.2f} MB, MLP {mem_diff_mlp:.2f} MB, interaction {mem_diff_interaction:.2f} MB")

                        last_time_look_up = dlrm.time_look_up
                        last_time_mlp = dlrm.time_mlp
                        last_time_interact = dlrm.time_interact
                        last_mem_look_up = dlrm.mem_look_up
                        last_mem_mlp = dlrm.mem_mlp
                        last_mem_interact = dlrm.mem_interact
                        total_dlrm = 0
                        total_dlrm_back=0
                        total_dlrm_backpass=0
    ...
    if args.enable_profiling:
        ...
        with open("dlrm_s_pytorch" + time_stamp + "_total.prof", "w") as prof_f:
            ...
            if prof.profile_memory:
                print_memory_usage(profile_data)
