...
def quantize_minmax(weights, bits=8):
    """Simple min-max quantization"""
    w_min, w_max = weights.min(), weights.max()
    scale = (w_max - w_min) / (2**bits - 1)
    scale = scale.clamp(min=1e-8)  # Avoid division by zero
    
    quantized = ((weights - w_min) / scale).round().clamp(0, 2**bits - 1).to(torch.uint8)
...
def dequantize_minmax(quantized, metadata):
    """Reverse min-max quantization"""
    return quantized.float() * metadata['scale'] + metadata['min']


def quantize_symmetric(weights, bits=8):
    """Symmetric quantization (better for weights centered around zero)"""
    abs_max = weights.abs().max()
    scale = abs_max / (2**(bits-1) - 1)  # 127 for 8-bit
    scale = scale.clamp(min=1e-8)
    
    quantized = (weights / scale).round().clamp(-2**(bits-1), 2**(bits-1) - 1)
    # Shift to [0, 255] for video codec
    quantized_uint8 = (quantized + 2**(bits-1)).to(torch.uint8)
...
def dequantize_symmetric(quantized, metadata):
    """Reverse symmetric quantization"""
    return (quantized.float() - metadata['zero_point']) * metadata['scale']
...
def quantize_asymmetric(weights, bits=8):
    ...
    quantized = ((weights / scale).round() + zero_point).clamp(qmin, qmax).to(torch.uint8)
...
def quantize_per_row(weights, bits=8):
    """Per-row quantization (best accuracy, more metadata)"""
    num_emb, emb_dim = weights.shape
    
    # Per-row min/max
    row_min = weights.min(dim=1, keepdim=True)[0]
    row_max = weights.max(dim=1, keepdim=True)[0]
    
    scale = (row_max - row_min) / (2**bits - 1)
    scale = scale.clamp(min=1e-8)
    
    quantized = ((weights - row_min) / scale).round().clamp(0, 2**bits - 1).to(torch.uint8)
    
    metadata = {
        'row_min': row_min.squeeze().cpu(),
        'row_max': row_max.squeeze().cpu(),
        'scale': scale.squeeze().cpu()
    }
    return quantized, metadata

def dequantize_per_row(quantized, metadata):
    """Reverse per-row quantization"""
    num_emb = quantized.shape[0]
    
    # Expand metadata to match tensor shape
    row_min = metadata['row_min'].view(num_emb, 1)
    scale = metadata['scale'].view(num_emb, 1)
    
    return quantized.float() * scale + row_min
class QSVEmbeddingCompressor:
    def __init__(self, 
                 codec='hevc_qsv',
                 quality=23,
                 quantization='asymmetric',  # 'minmax', 'symmetric', 'asymmetric', 'per_row'
                 bits=8):
        self.codec = codec
        ...
        # Quantization functions
        self.quantizers = {
            'minmax': (quantize_minmax, dequantize_minmax),
            'symmetric': (quantize_symmetric, dequantize_symmetric),
    ...
    def _encode_qsv(self, input_file, output_file, width, height):
        """Encode using QSV hardware encoder via ffmpeg"""
        ...
        cmd = [
            ...
            '-s', f'{width}x{height}',
            '-r', '1',  # 1 fps (we only have 1 "frame")
            '-i', input_file,
            '-c:v', self.codec,
            '-preset', 'medium',
            '-global_quality', str(self.quality),  # QSV quality
            '-look_ahead', '0',  # Disable lookahead for single frame
            '-frames:v', '1',
            output_file
        ]
        ...
        if result.returncode != 0:
            print(f"[QSV ERROR] Encoding failed!")
            print(f"Command: {' '.join(cmd)}")
            print(f"stderr: {result.stderr}")
            raise RuntimeError(f"QSV encoding failed")

    def _decode_qsv(self, input_file, output_file):
        """Decode using QSV hardware decoder via ffmpeg"""
        
        cmd = [
            'ffmpeg', '-y',
            '-hwaccel', 'qsv',  # Use QSV for decoding
            '-c:v', self.codec.replace('_qsv', ''),  # h264 or hevc
            '-i', input_file,
            '-pix_fmt', 'gray',
            '-f', 'rawvideo',
            output_file
        ...
        if result.returncode != 0:
            print(f"[QSV ERROR] Decoding failed!")
            print(f"stderr: {result.stderr}")
            raise RuntimeError(f"QSV decoding failed")
    def compress_table(self, embedding_weight):
        """
        CORRECTED: Explicit quantization before QSV encoding
        """
        weights = embedding_weight.detach().cpu()
        num_emb, emb_dim = weights.shape
        
        # STEP 1: QUANTIZE (Float32 → uint8)
        print(f"[QSV] Step 1: Quantizing {weights.shape} using {self.quantization}")
        quantize_fn, _ = self.quantizers[self.quantization]
        pixels_uint8, quant_metadata = quantize_fn(weights, bits=self.bits)
        
        # STEP 2: ENCODE with QSV (uint8 → compressed)
        print(f"[QSV] Step 2: Encoding with {self.codec}")
        ...
        # Metadata includes quantization params
        metadata = {
        ...
        # Report sizes
        original_size = weights.element_size() * weights.numel()  # Float32 bytes
        quantized_size = pixels_uint8.numel()  # uint8 bytes
        compressed_size = len(compressed_data)
        
        print(f"[QSV] Float32: {original_size/1e6:.2f}MB → "
              f"Quantized: {quantized_size/1e6:.2f}MB → "
              f"Compressed: {compressed_size/1e6:.2f}MB")
        print(f"[QSV] Total ratio: {original_size/compressed_size:.2f}x")
        
        return compressed_data, metadata
    
    def decompress_table(self, compressed_data, metadata):
        """
        CORRECTED: Decode + dequantize
        """
        num_emb, emb_dim = metadata['shape']
        
        # STEP 1: DECODE (compressed → uint8 pixels)
        with tempfile.TemporaryDirectory() as tmpdir:
            video_file = os.path.join(tmpdir, 'compressed.mp4')
            ...
            self._decode_qsv(video_file, raw_file)
            
            pixels_uint8 = np.fromfile(raw_file, dtype=np.uint8)
            pixels_uint8 = pixels_uint8.reshape(num_emb, emb_dim)
            pixels_uint8 = torch.from_numpy(pixels_uint8)
        
        # STEP 2: DEQUANTIZE (uint8 → Float32)
        _, dequantize_fn = self.quantizers[metadata['quantization']]
