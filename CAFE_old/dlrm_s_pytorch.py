...
# def print_sketch_timing_summary(lib, reset=True):
#     """Print detailed timing for sketch compression/decompression"""
#     if lib is None:
#         return
    
#     query_time = lib.get_query_time()
#     insert_val_time = lib.get_insert_val_time()
#     query_count = lib.get_query_count()
#     insert_val_count = lib.get_insert_val_count()
    
#     print("\n" + "="*70)
#     print("SKETCH COMPRESSION/DECOMPRESSION TIMING")
#     print("="*70)
    
#     print(f"\nDecompression (batch_query - hash lookup):")
#     print(f"  Total calls:  {query_count}")
#     print(f"  Total time:   {query_time:.6f} seconds")
#     if query_count > 0:
#         avg_time_us = (query_time / query_count) * 1e6
#         print(f"  Avg per call: {avg_time_us:.3f} µs")
#         print(f"  Note: Called in forward pass + once in backward pass")
    
#     print(f"\nCompression (batch_insert_val - sketch update):")
#     print(f"  Total calls:  {insert_val_count}")
#     print(f"  Total time:   {insert_val_time:.6f} seconds")
#     if insert_val_count > 0:
#         avg_time_us = (insert_val_time / insert_val_count) * 1e6
#         print(f"  Avg per call: {avg_time_us:.3f} µs")
#         print(f"  Note: Updates sketch with gradient norms")
    
#     # Calculate overhead percentage if both have been called
#     if query_count > 0 and insert_val_count > 0:
#         total_sketch_time = query_time + insert_val_time
#         print(f"\nTotal sketch overhead: {total_sketch_time:.6f} seconds")
#         print(f"  Query %:      {(query_time/total_sketch_time)*100:.1f}%")
#         print(f"  Insert_val %: {(insert_val_time/total_sketch_time)*100:.1f}%")
    
#     print("="*70 + "\n")
    
#     if reset:
#         lib.reset_sketch_timing()
...
#         # Setup timing function signatures
#         lib.reset_sketch_timing.argtypes = []
#         lib.reset_sketch_timing.restype = None
        
#         lib.get_query_time.argtypes = []
#         lib.get_query_time.restype = ctypes.c_double
        
#         lib.get_insert_val_time.argtypes = []
#         lib.get_insert_val_time.restype = ctypes.c_double
        
#         lib.get_query_count.argtypes = []
#         lib.get_query_count.restype = ctypes.c_longlong
        
#         lib.get_insert_val_count.argtypes = []
#         lib.get_insert_val_count.restype = ctypes.c_longlong
        
#         lib.print_sketch_timing.argtypes = []
#         lib.print_sketch_timing.restype = None
#     global dlrm
#     dlrm = DLRM_Net(
#         args.compress_rate,
#         args.ada_flag,
...
#                 ld_model = torch.load(args.load_model, weights_only=False, map_location=torch.device("cuda")
#                     # map_location=lambda storage, loc: storage.cuda(0)
#                 )
#         else:
#             # when targeting inference on CPU
#             ld_model = torch.load(args.load_model, weights_only=False, map_location=torch.device("cpu"))
#         dlrm.load_state_dict(ld_model["state_dict"])
#         if args.sketch_flag and not args.inference_only:
#             dlrm.lib.update()
#         ld_j = ld_model["iter"]
#         ld_k = ld_model["epoch"]
...
#                         total_samp = 0
#                         reset_sketch_time()
#                         if args.sketch_flag and lib is not None:
#                             print_sketch_timing_summary(lib, reset=True)
...
class DLRM_Net(nn.Module):
    ...
    def apply_emb(self, lS_o, lS_i, emb_l, v_W_l, test):
        ...
        # Start timer for embedding lookups
    ...
    def sequential_forward(self, dense_x, lS_o, lS_i, test):
        # process dense features (using bottom mlp), resulting in a row vector
        # === DEBUG: Check dense_x properties ===
        if not hasattr(self, '_debug_printed'):
            print(f"=== dense_x Debug ===")
            print(f"  dtype: {dense_x.dtype}")
            print(f"  shape: {dense_x.shape}")
            print(f"  contiguous: {dense_x.is_contiguous()}")
            print(f"  min/max: {dense_x.min():.4f} / {dense_x.max():.4f}")
...
def run():
    ...
    if args.data_generation == "dataset":
        ...
        train_data, train_ld, test_data, test_ld = dp.make_criteo_data_and_loaders(
            args)
        data_end = time.time()
        print(f"Data loading took: {data_end - data_start}s")
#        table_feature_map = {idx: idx for idx in range(len(train_data.counts))}
        nbatches = args.num_batches if args.num_batches > 0 else len(train_ld)
        nbatches_test = len(test_ld)
        ln_emb = train_data.counts
            ...
            while k < args.nepochs:
                ...
                for j, inputBatch in enumerate(train_ld):
                    ...
                    if should_print or should_test:
                        ...
                        print(f"Embedding lookup time: {time_diff_look_up:.2f} ms, "
                              f"MLP time: {time_diff_mlp:.2f} ms, "
                              f"interaction time: {time_diff_interact:.2f} ms\n"
                              f"Running dlrm: {time_dlrm:.2f} ms, "
                              f"backpropagation after dlrm: {time_dlrm_back_iteration:.2f} ms, "
                              f"backpass: {time_dlrm_backpass_iteration:.2f} ms")

                        # Update last timing values
                        ...
                        total_samp = 0
                        if args.sketch_flag and lib is not None:
                            # Report batch_insert_val call counts (before reset)
                            total_insert_calls = sum(
                                e.insert_grad_call_count
                                for e in dlrm.emb_l
                                if hasattr(e, 'insert_grad_call_count')
                            )
                            print(f"[sketch] Total Python insert_grad calls: {total_insert_calls}")
                            print_sketch_timing_summary(lib, reset=True)
    ...
    if args.sketch_flag:
        print("\n" + "="*70)
        print("DETAILED SKEmbeddingBag TIMING BREAKDOWN")
        print("="*70)
        for k, emb in enumerate(dlrm.emb_l):
            if dlrm.sketch_emb[k]:
                emb.print_timing_summary()
