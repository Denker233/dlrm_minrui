...
class DLRM_Net(nn.Module):
    ...
    # def create_emb(self, m, ln, weighted_pooling=None, merge_threshold=10000):
    #     emb_l = nn.ModuleList()
    #     v_W_l = []
    ...
    #         # Only merge small tables that don't need QR/MD
    #         needs_special_treatment = (
    #             (self.qr_flag and n > self.qr_threshold) or 
    #             (self.md_flag and n > self.md_threshold)
    #         )
            
    #         if n < merge_threshold and not needs_special_treatment:
    #             current_group.append((i, n))
    #             current_group_size += n
    #             if current_group_size > merge_threshold * 5:
    #                 merge_groups.append(current_group)
    #                 current_group = []
    #                 current_group_size = 0
    #         else:
    #             standalone_tables.append((i, n))
        
    #     if current_group:
    #         merge_groups.append(current_group)
        
    #     # Create merged embedding tables (for small tables only)
    #     self.emb_index_map = {}
    #     emb_idx = 0
        
    #     for group in merge_groups:
    #         total_size = sum(n for _, n in group)
    #         # Merged tables use standard EmbeddingBag only
    ...
    #             self.emb_index_map[orig_idx] = (emb_idx, offset)
    #             offset += n
            
    #         emb_l.append(EE)
    #         v_W_l.append(None if weighted_pooling is None else torch.ones(total_size, dtype=torch.float32))
    #         emb_idx += 1
        
    #     # Create standalone tables (keeps ALL original optimizations)
    #     for orig_idx, n in standalone_tables:
    #         # QR branch - for very large tables
    ...
    #             )
    #         # MD branch - for medium-large tables
    ...
    #         # Standard branch - for normal-sized tables
    #         else:
    ...
    #         self.emb_index_map[orig_idx] = (emb_idx, 0)
    #         emb_l.append(EE)
    #         v_W_l.append(None if weighted_pooling is None else torch.ones(n, dtype=torch.float32))
    #         emb_idx += 1
        
    #     return emb_l, v_W_l
    def create_emb(self, m, ln, weighted_pooling=None, merge_threshold=0, min_table_size=50):
        emb_l = nn.ModuleList()
        v_W_l = []
        ...
        # If merge_threshold is 0, use original logic (no merging)
        if merge_threshold == 0:
            print("[MERGE] Merge threshold is 0 - using original embedding creation (no merging)")
            for i in range(0, ln.size):
                if ext_dist.my_size > 1:
                    if i not in self.local_emb_indices:
                        continue
                ...
                # Map each feature to its own table (no merging)
            ...
            print(f"[MERGE] Created {len(emb_l)} separate embedding tables (no merging)")
            return emb_l, v_W_l
        
        # Merging logic when merge_threshold > 0
        print(f"\n[MERGE] Merging embedding tables")
        print(f"[MERGE] Min table size: {min_table_size} (tables below this stay standalone)")
        print(f"[MERGE] Merge threshold: {merge_threshold} (tables below this get merged)")
        print(f"[MERGE] Strategy: Only merge tables with size >= {min_table_size} AND < {merge_threshold}")
        ...
        tiny_count = 0      # < min_table_size (too small to merge, stay standalone)
        medium_count = 0    # >= min_table_size and < merge_threshold (will be merged)
        large_count = 0     # >= merge_threshold (stay standalone)
        ...
        for i in range(0, ln.size):
            ...
            # Check if table needs special treatment (QR/MD)
            needs_special_treatment = (
                (self.qr_flag and n > self.qr_threshold) or 
                (self.md_flag and n > self.md_threshold)
            )
            
            # Only merge medium-sized tables that don't need QR/MD
            # Tables < min_table_size stay standalone (too small)
            # Tables >= min_table_size and < merge_threshold get merged (medium)
            # Tables >= merge_threshold stay standalone (large)
            
            is_medium_sized = n >= min_table_size and n < merge_threshold
            
            if is_medium_sized and not needs_special_treatment:
                medium_count += 1
                current_group.append((i, n))
                current_group_size += n
                # If merged group gets too large, finalize it
                if current_group_size > merge_threshold * 5:
                    merge_groups.append(current_group)
                    current_group = []
                    current_group_size = 0
            else:
                # Keep as standalone (either too small, too large, or needs special treatment)
                if n < min_table_size:
                    tiny_count += 1
                else:
                    large_count += 1
        ...
        print(f"[MERGE] Total tables: {ln.size}")
        print(f"[MERGE] Tiny tables (< {min_table_size}, kept standalone): {tiny_count}")
        print(f"[MERGE] Medium tables (>= {min_table_size} and < {merge_threshold}, will merge): {medium_count}")
        print(f"[MERGE] Large tables (>= {merge_threshold}, kept standalone): {large_count}")
        print(f"[MERGE] Number of merge groups: {len(merge_groups)}")
        
        # Create merged embedding tables (for medium-sized tables)
        emb_idx = 0
        
        for group_num, group in enumerate(merge_groups):
            total_size = sum(n for _, n in group)
            table_sizes = [n for _, n in group]
            print(f"[MERGE] Group {group_num}: merging {len(group)} tables, sizes={table_sizes}, total size={total_size}")
            
            # Apply QR/MD optimizations to merged table if it meets thresholds
            if self.qr_flag and total_size > self.qr_threshold:
                print(f"[MERGE]   -> Using QR embedding for merged table (size={total_size} > threshold={self.qr_threshold})")
                EE = QREmbeddingBag(
                    total_size,
                    m,
                    self.qr_collisions,
                ...
                print(f"[MERGE]   -> Using MD embedding for merged table (size={total_size} > threshold={self.md_threshold})")
                base = max(m) if isinstance(m, list) else m
                # For merged tables, use base dimension
                _m = base
                ...
                print(f"[MERGE]   -> Using standard EmbeddingBag for merged table")
                # Standard EmbeddingBag for merged tables
                EE = nn.EmbeddingBag(total_size, m, mode="sum", sparse=True)
        ...
        # Create standalone tables (includes tiny tables, large tables, and tables needing special treatment)
        for orig_idx, n in standalone_tables:
            # QR branch - for very large tables
            if self.qr_flag and n > self.qr_threshold:
                print(f"[MERGE] Table C{orig_idx+1} (size={n}): Using QR embedding (standalone)")
                EE = QREmbeddingBag(
                    n, m, self.qr_collisions,
                    operation=self.qr_operation,
                    mode="sum", sparse=True,
            ...
            # MD branch - for medium-large tables
                ...
                print(f"[MERGE] Table C{orig_idx+1} (size={n}): Using MD embedding (standalone)")
                base = max(m) if isinstance(m, list) else m
                _m = m[orig_idx] if isinstance(m, list) and n > self.md_threshold else base
                EE = PrEmbeddingBag(n, _m, base)
                W = np.random.uniform(
            ...
            # Standard branch - for normal-sized tables
                ...
                reason = "too small" if n < min_table_size else "large enough"
                print(f"[MERGE] Table C{orig_idx+1} (size={n}): Using standard EmbeddingBag (standalone, {reason})")
                EE = nn.EmbeddingBag(n, m, mode="sum", sparse=True)
                W = np.random.uniform(
                    low=-np.sqrt(1 / n), high=np.sqrt(1 / n), size=(n, m)
                ).astype(np.float32)
        ...
        print(f"[MERGE] Final number of tables: {len(emb_l)}")
        print(f"[MERGE] Reduction: {ln.size} -> {len(emb_l)} ({ln.size - len(emb_l)} tables saved)\n")
    ...
    def __init__(
        ...
        if (
            ...
            # create operators
            if ndevices <= 1:
                self.emb_l, w_list = self.create_emb(m_spa, ln_emb, weighted_pooling, merge_threshold)
    ...
    def apply_emb(self, lS_o, lS_i, emb_l, v_W_l):
        ...
        # Start timer for embedding look ups
        ...
        for k, sparse_index_group_batch in enumerate(lS_i):
            ...
            # NEW: Handle merged embedding tables
            if hasattr(self, 'emb_index_map') and k in self.emb_index_map:
                emb_idx, offset = self.emb_index_map[k]
                # Apply offset to indices for merged tables
                sparse_index_group_batch = sparse_index_group_batch + offset
            else:
                # Fallback: use original index if no mapping exists
                emb_idx = k
            ...
            if v_W_l[emb_idx] is not None:
                per_sample_weights = v_W_l[emb_idx].gather(0, sparse_index_group_batch)
            else:
                per_sample_weights = None

            # Handle quantized embeddings
            if self.quantize_emb:
                s1 = self.emb_l_q[emb_idx].element_size() * self.emb_l_q[emb_idx].nelement()
                s2 = self.emb_l_q[emb_idx].element_size() * self.emb_l_q[emb_idx].nelement()
                print("quantized emb sizes:", s1, s2)

                if self.quantize_bits == 4:
                    QV = ops.quantized.embedding_bag_4bit_rowwise_offsets(
                        self.emb_l_q[emb_idx],
