#!/usr/bin/env bash
set -euo pipefail
# DLRM Merge Table Performance Testing
...
BATCH_SIZE="${BATCH_SIZE:-16380}"
SPARSE_DIM="${SPARSE_DIM:-64}"
LOOKUPS="${LOOKUPS:-64}"

# Test different merge thresholds
MERGE_THRESHOLDS="${MERGE_THRESHOLDS:-0 1000 10000 100000}"
...
OUTDIR="${OUTDIR:-./16380_merge_performance_logs}"
...
run_mbm_monitor() {
  ...
  if [[ -f "$inference_log" ]]; then
    ...
    # Extract throughput
    grep -i "throughput\|samples/sec\|it/s" "$inference_log" | tail -5 | tee -a "$mbm_summary" || true
    
    # Extract timing information
    grep -i "time taken\|elapsed\|inference time\|total time" "$inference_log" | tail -5 | tee -a "$mbm_summary" || true
    
    # Extract embedding lookup timing if available
    grep -i "look up emb\|embedding.*time" "$inference_log" | tail -5 | tee -a "$mbm_summary" || true
  fi
...
echo "========================================"
echo "DLRM Merge Table Performance Testing"
...
for T in $THREADS_LIST; do
  ...
  for THRESHOLD in $MERGE_THRESHOLDS; do
    ...
    existing_logs=("$OUTDIR"/merge_T${T}_B${BATCH_SIZE}_D${SPARSE_DIM}_L${LOOKUPS}_THRESH${THRESHOLD}_*.mbm_summary.txt)
    ...
    logfile="$OUTDIR/merge_T${T}_B${BATCH_SIZE}_D${SPARSE_DIM}_L${LOOKUPS}_THRESH${THRESHOLD}_${stamp}.log"
    ...
    if run_mbm_monitor "$MBM_BASE" "${cmd[@]}"; then
      ...
      echo "[WARN] Failed T=$T THRESH=$THRESHOLD — see ${MBM_BASE}.inference.log" >&2
    fi
...
SUMMARY_FILE="$OUTDIR/merge_performance_summary.txt"
echo "DLRM Merged Embedding Table Performance Comparison" > "$SUMMARY_FILE"
...
for T in $THREADS_LIST; do
  ...
  for THRESHOLD in $MERGE_THRESHOLDS; do
    ...
    summary_files=("$OUTDIR"/merge_T${T}_B${BATCH_SIZE}_D${SPARSE_DIM}_L${LOOKUPS}_THRESH${THRESHOLD}_*.mbm_summary.txt)
    if [[ -f "${summary_files[0]}" ]]; then
      grep "PEAK_GBps\|AVG_GBps\|L1_MISS_RATE\|LLC_MISS_RATE\|Time taken" "${summary_files[0]}" >> "$SUMMARY_FILE" 2>/dev/null || echo "No metrics found" >> "$SUMMARY_FILE"
    else
