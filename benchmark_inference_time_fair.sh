...
echo "========================================="
echo "DLRM Inference Time Benchmark (Fair)"
echo "========================================="
...
clear_caches() {
    ...
    if sudo -n sh -c 'echo 3 > /proc/sys/vm/drop_caches' 2>/dev/null; then
        ...
        echo "  Sleeping 30 seconds to let system settle..."
        sleep 30
    fi
    echo ""
}
...
print("INFERENCE TIME (Full Test Dataset, with cache clearing):")
...
if abs(time_pct) < 1:
    ...
    if time_pct < 0:
        print(f"  ✓ Compressed is FASTER: {abs(time_pct):.2f}%")
    else:
